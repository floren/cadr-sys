...
(PROCLAIM '(SPECIAL DESTINATION-CONTEXT LOCALITY I-MEM-LOC D-MEM-LOC
		    A-MEM-CREVICE-LIST A-CONSTANT-LOC M-CONSTANT-LOC
		    CONSLP-INPUT CONSLP-OUTPUT
		    VERSION-NUMBER
		    BASE-VERSION-NUMBER		;NIL or, if incremental assembly,
						; version this to augment.
		    ...
		    MC-LINKAGE-ALIST
		    COLD-LOAD-AREA-SIZES PAGE-SIZE CONS-LAP-PASS2
		    MICRO-CODE-SYMBOL-TABLE-FILL-VALUE
		    CONS-LAP-INIT-STATE		;If this is non-null, current assembly is
						; incremental from this saved state.
		    CURRENT-ASSEMBLY-MICRO-ENTRIES	;List, each element is
						; (<type> <name> <adr>), in incremental assbly
  ...
  (PROG (TEM) 
	...
	(SETQ M-CONSTANT-LIST			;DUMMY UP SLOTS FOR USAGE COUNT AND LAST
	      (COND ((SETQ TEM (GETF INIT-STATE 'M-CONSTANT-LIST))  ;USE
		     (MAPCAR #'(LAMBDA (X) (APPEND X '(100000 NIL) NIL))
			     TEM))
		    (T NIL)))
...
(DEFUN ASSEMBLE (&OPTIONAL FN INIT-STATE DONT-RE-READ &AUX INPUT-FILE INPUT-TRUENAME)
  (LET ((*READ-BASE* 8) (*PRINT-BASE* 8) (*READTABLE* SI:INITIAL-READTABLE))
    ...
    (COND ((NOT (NUMBERP VERSION-NUMBER))
	   (FORMAT T "~&Enter version number: ")
	   (SETQ VERSION-NUMBER (READ))))
      (LET ((TIME (TIME))
	...
	(FORMAT T "~&Assembly time ~D:~2,'0D, ~D disk reads, ~D disk writes~%"
		(TRUNCATE TIME 3600.) (\ (TRUNCATE TIME 60.) 60.)
		(- (READ-METER 'SI:%COUNT-DISK-PAGE-READS) DR)
		(- (READ-METER 'SI:%COUNT-DISK-PAGE-WRITES) DW))))))
...
;    (OR (EQ (SEND INPUT-FILE-1 :HOST) (FS:GET-PATHNAME-HOST "SYS"))
...
;	 (SETQ INPUT-FILE (SEND INPUT-FILE-1 :NEW-DIRECTORY '("UBIN")))))
  (SETQ CONSLP-OUTPUT-PATHNAME (SEND INPUT-FILE :NEW-PATHNAME
...
(PROCLAIM '(SPECIAL SI::*FILES-TO-MICRO-ASSEMBLE*))
...
(DEFUN MICRO-ASSEMBLE-SYSTEM-DO-IT ()
  (LET* ((FILE-TRUENAMES (MAPCAR #'(LAMBDA (X) (SEND X :TRUENAME))
	 ...
	 (OUTPUT-MCR-PATHNAME (SEND OUTPUT-GENERIC-PATHNAME
				    :NEW-CANONICAL-TYPE :CADR-MICROCODE))
    ...
    (COND ((ZEROP OLD-VERSION-NUMBER)
	   (FORMAT T "~&Enter version number: ")
	   (SETQ VERSION-NUMBER (READ)))
	  (T (SETQ VERSION-NUMBER (1+ OLD-VERSION-NUMBER))))
...
(DEFUN LISTIFY-PATHNAME (PATHNAME)
  (LIST (SEND (SEND PATHNAME :HOST) :NAME)
  ...
  (PROG (;I-MEM-LOC D-MEM-LOC A-MEM-LOC M-MEM-LOC M-CONSTANT-LOC A-CONSTANT-LOC ;USE TOP LEVEL
	...
	(DOLIST (*UA-TRUENAME* FILE-TRUENAMES)
	  (LET ((BEG-I-MEM-LOC I-MEM-LOC))
	    (DOLIST (S (SEND *UA-TRUENAME* :GET 'UA-SEXP))
	      (CONS-LAP-PASS1 S))
...
(DEFUN WRITE-ERROR-TABLE (FN)
  (LET ((*READ-BASE* 8) (*PRINT-BASE* 8)
    ...
    (WITH-OPEN-FILE (OUTPUT-FILE FN :direction :output :characters t)
...
(DEFUN CONS-LAP-REPORT-CONSTANTS-USAGE (L)
  (SETQ L (SORT (COPYLIST L) #'(LAMBDA (X Y) (< (CADDR X) (CADDR Y)))))
  (TERPRI)
		...
		(GO X))
	      ...
	      ((EQ (CAR WD) 'IF)
	       (COND ((si:eval-special-ok (CADR WD))
		     ...
		     (T (MAPC #'CONS-LAP-PASS1 (CDDDR WD)))))
...
(DEFUN CONS-LAP-D-MEM-LOC (L)
  (OR CONS-LAP-PASS2
      (DO ((BL D-MEM-FREE-BLOCKS (CDR BL))
	   (TEM))
	  ((NULL (CDR BL)) (FERROR NIL 'CONS-LAP-D-MEM-LOC))
...
(DEFUN CONS-LAP-REPEAT-1 (COUNT LST)
  (PROG (ORPCNT RPCNT)
	...
	(MAPC #'(LAMBDA (X) (CONS-LAP-PASS1 (COND ((ATOM X) (LIST X))
						  (T X))))
	      LST)
	(SETQ COUNT (1- COUNT))
	(SETQ RPCNT (1+ RPCNT))
	...
	(COND (CONS-LAP-INSIDE-COMMENT
		 ...
		 (COND ((NULL OPCODE)
	      ...
	      ((AND (EQ (CAR WD) 'MC-LINKAGE)
		    (LISTP (CADR WD)))
	       (MAPC #'CONS-LAP-MC-LINKAGE-STORE (CADR WD)))
	      ...
	      ((EQ (CAR WD) 'IF)
	       (COND ((si:eval-special-ok (CADR WD))
		     ...
		     (T (MAPC #'CONS-LAP-PASS2 (CDDDR WD)))))
	      (T (GO W1)))
     X	(RETURN NIL)
     W1	(SETQ CONS-LAP-WDS-SINCE-LAST-SYM (1+ CONS-LAP-WDS-SINCE-LAST-SYM))
	(COND (DISPATCH-ARM 
...
(DEFUN CONS-LAP-MC-LINKAGE-VALUE (MEMORY SYMBOL)
  (PROG (V MULT)
	(COND ((NULL (SETQ V (ASS #'STRING-EQUAL SYMBOL MC-LINKAGE-ALIST)))
	       (FERROR NIL "~%Undefined MC-LINKAGE symbol ~S" SYMBOL)))
	(SETQ V (CADDR V))
	(COND ((STRING-EQUAL MEMORY "NUMBER") (GO X))
	      ((SETQ MULT (ASS #'STRING-EQUAL MEMORY
  ...
  (PROG (ORPCNT RPCNT)
	...
	(SETQ RPCNT 0)
     L	(COND ((ZEROP COUNT)
	       ...
	       (RETURN NIL)))
	(CONS-LAP-SET 'REPEAT-COUNT RPCNT)
	(MAPC #'(LAMBDA (X) (CONS-LAP-PASS2 (COND ((ATOM X) (LIST X))
						  (T X))))
	      LST)
	(SETQ COUNT (1- COUNT))
	(SETQ RPCNT (1+ RPCNT))
...
(DEFUN ADD-FIELD-INDICATORS (X)
  (PROG ()
	...
	(COND ((EQ X 'A-MEM)
		...
		(GO F-J)))
     X
	(COND ((NOT (MEMQ X FIELD-INDICATORS))
		(SETQ FIELD-INDICATORS (CONS X FIELD-INDICATORS))))
	(RETURN NIL)
     F-B
	(COND ((MEMQL '(I-MEM D-MEM) COMBINED-INDICATORS)
	       (GO E1)))
	(GO X)
     F-A
	(COND ((OR (MEMQ INSTRUCTION-CONTEXT '(FORCE-DISPATCH FORCE-JUMP))
		   (MEMQL '(I-MEM D-MEM) COMBINED-INDICATORS))
	       (GO E1)))
	(GO X)
     F-J
     ADD-I
	(COND ((MEMQ INSTRUCTION-CONTEXT '(FORCE-DISPATCH FORCE-BYTE FORCE-ALU))
	       (GO E1)))
	(GO X)
     F-D
     ADD-D
	(COND ((OR (MEMQ INSTRUCTION-CONTEXT '(FORCE-JUMP FORCE-BYTE FORCE-ALU))
		   (MEMQL '(I-MEM) COMBINED-INDICATORS))  ;A-MEM OK NOW IF WRITING DRAM
	       (GO E1)))
	(GO X)
     E1
	(CONS-LAP-BARF (LIST X FIELD-INDICATORS COMBINED-INDICATORS)
	      'INDICATOR-CONFLICT 
	      'DATA)
	(RETURN NIL)
))
