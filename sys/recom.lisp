...
(DEFUN RECOMPILE-FILES-AFTER-DATE (AFTER-DATE &OPTIONAL (SYSTEM "System")
				   &AUX RECOM-FILES TEM)
  (AND (ERRORP (SETQ AFTER-DATE (TIME:PARSE-UNIVERSAL-TIME AFTER-DATE 0 NIL NIL)))
       (RESIGNAL-ERROR AFTER-DATE))
...
(DEFUN RECOMPILE-FILES-BEFORE-DATE (BEFORE-DATE &OPTIONAL (SYSTEM "System")
				    &AUX RECOM-FILES TEM)
  (AND (ERRORP (SETQ BEFORE-DATE (TIME:PARSE-UNIVERSAL-TIME BEFORE-DATE 0 NIL NIL)))
       (RESIGNAL-ERROR BEFORE-DATE))
  (DOLIST (P (QFASL-SOURCE-FILE-PLISTS SYSTEM))
...
(DEFUN SAME-FILE-P (F1 F2)
  (OR (EQ F1 F2)
      (AND (EQUALP (FUNCALL F1 ':DIRECTORY) (FUNCALL F2 ':DIRECTORY))
	   (EQUALP (FUNCALL F1 ':NAME) (FUNCALL F2 ':NAME))
	   (EQUALP (FUNCALL F1 ':VERSION) (FUNCALL F2 ':VERSION))
	   (OR (EQUALP (FUNCALL F1 ':TYPE) (FUNCALL F2 ':TYPE))
...
(DEFUN RECOMPILE-SOURCE-FILES (SOURCE-FILE-LIST)
  (DOLIST (FILE SOURCE-FILE-LIST)
    ...
    ;; On Twenex you get an error trying to recompile a file
    ;; if a QFASL exists already.
    (WITH-OPEN-FILE (STR FILE ':PROBE)
      (DELETEF (FUNCALL (FUNCALL STR ':TRUENAME) ':NEW-TYPE "QFASL") NIL))
    (QC-FILE FILE)))
