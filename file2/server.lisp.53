...
(DEFUN FS:CONSIDER-UNIT-OK (UNIT BAND)
  "Just like fs:consider-unit T, but we also inform the user."
  (AND UNIT ;;should never be NIL
       (NOT (CONDITION-CASE (DISK-ERROR)
	   (*CATCH 'FS:LMFILE-ERROR
...
;;  "Will restart standard file services.   FOR-WARM-BOOT should be NIL when
...
When it is up again, please call BRING-UP-SERVER."  (si:get-site-option ':sys-host))))
	 ...
	 (cond ((or (FS:CONSIDER-UNIT-OK 2 "SRVR")
		  (FS:CONSIDER-UNIT-OK 0 "SERV"))
		(FS:START-FILE-SYSTEM)
		(UNLESS (GET 'LMFILE-CHAOS-HOST 'SI:FLAVOR)
		  (FS:ADD-THIS-LFS-HOST) ;(FS:ADD-LFS-HOST "FC"))
