...
(DEFUN NEW-SYSTEM (&OPTIONAL (RECOMPILEP T) (OUTERP NIL) &AUX START-TIME END-TIME)
  ...
  (SI:FULL-GC)  (IF RECOMPILEP (RECOMPILE-AND-LOAD-SYSTEM-DEFS)   (LOAD-SYSTEM-DEFS))
...
(DEFVAR SYSTEM-DEFINITION-FILES '("SYS:SYS2;PRODEF"	 
				  "SYS:IO;RDDEFS"	 
				  "SYS:SYS2;SGDEFS"	 
				  "SYS:SYS2;NUMDEF"))

(DEFUN LOAD-SYSTEM-DEFS ()
  (MAPCAR #'(LAMBDA (FILE)
	    (IF (PROBEF (SEND (FS:PARSE-PATHNAME FILE) :NEW-TYPE :QFASL))
		(LOAD FILE)
		(QC-FILE-LOAD FILE)))
	  SYSTEM-DEFINITION-FILES))

(DEFUN RECOMPILE-AND-LOAD-SYSTEM-DEFS ()
  (QC-FILES SYSTEM-DEFINITION-FILES T))
...
(DEFUN COMPILE-SYSTEM-1 (SYSTEM &OPTIONAL (RECOMPILEP NIL))
  (IF RECOMPILEP
...
(DEFUN COMPILE-SYSTEM (SYSTEM &OPTIONAL (RECOMPILEP NIL))
  (IF (LISTP SYSTEM)
      (DOLIST (S SYSTEM) (COMPILE-SYSTEM-1 S RECOMPILEP))
      (COMPILE-SYSTEM-1 SYSTEM RECOMPILEP)))