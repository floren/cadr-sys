...
;background-bits added to keep from receiving all packets - not tested yet ...  -pace
...
(defvar *network-address-alist* nil
  "alist of (chaos-address . ethernet-address) for each host we know about")

(defun define-ethernet-chaos-host (chaos-address ethernet-address)
  (let ((old-entry (assoc chaos-address *network-address-alist*)))
    (cond (old-entry (rplacd old-entry ethernet-address))
	  (t
	   (push (cons chaos-address ethernet-address) *network-address-alist*)))))

;;; below are the entries for LMI's machines.  You will probably want to replace them with
;;; your own.  When the Plummer address resolution protocol is implemented, this will no
;;; longer be needed as address conversion will be negotiated over the network.

(define-ethernet-chaos-host 3140
			    #x02608c001621)	;lm 2
;(define-ethernet-chaos-host 3034
;			    #x02608c001463)	;lm 3
;(define-ethernet-chaos-host 3050
;			    #x02608c001463)	;lm 1
(define-ethernet-chaos-host 3410
			    #X2608C001116	;lambda board now at LMI
			  ; #x02608c000546	;lambda
  )
...
;  (compiler:%multibus-write-8 3com-mecsr new-csr)
;  (compiler:%multibus-write-8 (1+ 3com-mecsr) (ldb 1010 new-csr)))
(defun write-3com-csr (new-csr)
  (if *lambda-has-ethernet*
      (%nubus-write #xff 3com-mecsr new-csr)  ;this also writes the jam-backoff counter, which
		;I guess doesnt hurt.  Writing it in two pieces also has potential lossage.
    (ferror nil "no ethernet is present")))

;(defun read-3com-csr ()
;  (dpb (compiler:%multibus-read-8 (1+ 3com-mecsr))
;       1010
;       (compiler:%multibus-read-8 3com-mecsr)))
(defun read-3com-csr ()
  (if *lambda-has-ethernet*
      (ldb 0020 (%nubus-read #xff 3com-mecsr))
    (ferror nil "no ethernet is present")))
...
(defun load-address-reverse ()
  (cond (*lambda-has-ethernet*
	 (dotimes (i 6)
	   (let ((next-byte (compiler:%multibus-read-8 (+ 3com-address-rom i))))
	     (compiler:%multibus-write-8 (+ 3com-address-ram (- 5 i)) next-byte)
	     (setq my-ethernet-address (dpb next-byte 0010 (ash my-ethernet-address 8.))))))
	(t
	 (ferror nil "no ethernet present")))
  )
...
(defun write-lambda-3com-frame-header (buffer-base offset source destination type)
 ...
 ;	(type 0))
  ...
  ; (setq destination (dpb (compiler:%multibus-read-8 (+ buffer-base 2)) 5010 destination))
  ; (setq destination (dpb (compiler:%multibus-read-8 (+ buffer-base 3)) 4010 destination))
  ; (setq destination (dpb (compiler:%multibus-read-8 (+ buffer-base 4)) 3010 destination))
  ; (setq destination (dpb (compiler:%multibus-read-8 (+ buffer-base 5)) 2010 destination))
  ; (setq destination (dpb (compiler:%multibus-read-8 (+ buffer-base 6)) 1010 destination))
  ; (setq destination (dpb (compiler:%multibus-read-8 (+ buffer-base 7)) 0010 destination))

  ; (setq source (dpb (compiler:%multibus-read-8 (+ buffer-base 10)) 5010 source))
  ; (setq source (dpb (compiler:%multibus-read-8 (+ buffer-base 11)) 4010 source))
  ; (setq source (dpb (compiler:%multibus-read-8 (+ buffer-base 12)) 3010 source))
  ; (setq source (dpb (compiler:%multibus-read-8 (+ buffer-base 13)) 2010 source))
  ; (setq source (dpb (compiler:%multibus-read-8 (+ buffer-base 14)) 1010 source))
  ; (setq source (dpb (compiler:%multibus-read-8 (+ buffer-base 15)) 0010 source))
    
  ; (dotimes (i 2)
  ;   (setq type (dpb (compiler:%multibus-read-8 (+ buffer-base i 16))
  ;		      0010
  ;		      (ash type 8))))
...
(defun read-lambda-3com-frame-header (buffer-base)
  ...
  (if (null *lambda-has-ethernet*) (ferror nil "no ethernet present"))
  (if (and break-p
	   (neq current-stack-group si:scheduler-stack-group))
      (cerror ':no-action nil nil "foo"))
  (dpb (%multibus-read-8 (+ buffer-base 17))
       1010
       (%multibus-read-8 (+ buffer-base 16))))

(defun p (&optional (buf-base 614000))
  (dotimes (i 30)
