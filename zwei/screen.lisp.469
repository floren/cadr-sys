...
(DEFUN MAKE-MENU-COMMAND-1 (ALIST-ELT)
  (IF (STRING-EQUAL (CAR ALIST-ELT) "Mouse " :END1 6)
...
(DEFFLAVOR ZWEI
  ...
  (:DOCUMENTATION "Mixin for editor windows"))
...
(DEFMETHOD (ZWEI :VERIFY-NEW-EDGES) (NEW-LEFT NEW-TOP NEW-WIDTH NEW-HEIGHT)
  (DECLARE (IGNORE NEW-LEFT NEW-TOP NEW-WIDTH))
...
;;w; ithout going through the editor stream mechanism for either input or output.
...
(DEFMETHOD (EDITOR-TYPEOUT-WINDOW :MORE-TYI) (&AUX CH)
  (DO-FOREVER
    ...
    (WHEN (OR (NUMBERP CH)			;characterp
	      (EQ (CAR-SAFE CH) ':TYPEOUT-EXECUTE))
      (RETURN CH))))
...
(DEFVAR *MOUSE-FONT-CHAR* 25.)			;NE arrow by default
...
(DEFMETHOD (TYPEIN-WINDOW :PRINT-NOTIFICATION)
  ...
  (IF (MULTIPLE-VALUE-BIND (NIL NIL FINAL-INDEX)
	...
	FINAL-INDEX)
      ...
      (TV:POP-UP-NOTIFY :PRINT-NOTIFICATION TIME STRING WINDOW-OF-INTEREST)
  ...
  (AND (SETQ NLINES (GET PLIST ':NUMBER-OF-MINI-BUFFER-LINES))
       (PUTPROP PLIST
		(+ 4  ;Room for margins of mini buffer window.
		   (* (1+ NLINES)
		      (+ (MAX (TV:FONT-CHAR-HEIGHT
				(TV:SCREEN-DEFAULT-FONT (TV:SHEET-GET-SCREEN TV:SUPERIOR)))
			      (TV:FONT-CHAR-HEIGHT FONTS:SEARCH))
			 2)))
		':INSIDE-HEIGHT)))
...
(DEFMETHOD (MODE-LINE-SUPERIOR-MIXIN :AFTER :INIT) (INIT-PLIST)
  (LET ((MINI-BUFFER-FLAVOR
    ...
    (SETQ TYPEIN-WINDOW
	  ...
	  MINI-BUFFER-WINDOW
	    (MAKE-INSTANCE MINI-BUFFER-FLAVOR
			   :TOP (+ (TV:SHEET-INSIDE-TOP) TV:LINE-HEIGHT)
	  ...
	  SEARCH-MINI-BUFFER-WINDOW
	    (MAKE-INSTANCE MINI-BUFFER-FLAVOR
			   ...
			   :EDITOR-CLOSURE-VARIABLES
			     (GET INIT-PLIST
				  ':MINI-BUFFER-EDITOR-CLOSURE-VARIABLES)
			   :LABEL NIL
			   :SAVE-BITS NIL
			   :ACTIVATE-P T))))
...
(DEFUN CREATE-MODE-LINE-WINDOW (&OPTIONAL (NLINES 3) (SUPERIOR TV:DEFAULT-SCREEN)
				...
				IO-BUFFER
				&REST OPTIONS)
  (APPLY #'MAKE-INSTANCE MODE-LINE-FLAVOR :NUMBER-OF-MINI-BUFFER-LINES NLINES
					  :SUPERIOR SUPERIOR
					  :IO-BUFFER IO-BUFFER
					  OPTIONS))
...
(DEFMETHOD (TEMPORARY-MODE-LINE-WINDOW :MOVE-NEAR-WINDOW) (WINDOW &OPTIONAL (EXPOSE-P T))
  ...
  (MULTIPLE-VALUE-BIND (LEFT TOP RIGHT BOTTOM)
    ...
    (AND (> (+ BOTTOM TV:HEIGHT) (TV:SHEET-INSIDE-BOTTOM TV:SUPERIOR))
	 (SETQ BOTTOM (MAX (- TOP TV:HEIGHT) (TV:SHEET-INSIDE-TOP TV:SUPERIOR))))
    ;; Put it there
...
;	    (*IO-BUFFER* TV:IO-BUFFER)
  ...
  (COND ((EQ WINDOW ':MOUSE)
	 (SEND SELF ':SET-SIZE #o700 (TV:SHEET-HEIGHT SELF))
  ...
  (UNWIND-PROTECT
    (PROGN
      ...
      (LET ((TV:SELECTED-IO-BUFFER TV:IO-BUFFER))
	(TV:KBD-SNARF-INPUT TV:IO-BUFFER))
	 ...
	 (USING-RESOURCE (WINDOW BACKGROUND-TYPEOUT-WINDOWS)
	   (LET ((WO (SEND WINDOW :WHICH-OPERATIONS)))
	     (SETQ *BACKGROUND-TYPEOUT-WHICH-OPERATIONS*
		   (SI:UNION-EQ '(:BEEP :NEVER-FLUSH-TYPEOUT :INHIBIT-OUTPUT-FOR-ABORT-P)
				WO)))))
    ...
    (:BEEP
     (LET ((W (WITHOUT-INTERRUPTS
		(IF TV:SELECTED-WINDOW
		    (TV:SHEET-GET-SCREEN TV:SELECTED-WINDOW)
		    TV:DEFAULT-SCREEN))))
...
(DEFFLAVOR BACKGROUND-TYPEOUT-WINDOW
	()
	(TV:ANY-TYI-MIXIN TV:BASIC-MOUSE-SENSITIVE-ITEMS TV:TEMPORARY-WINDOW-MIXIN TV:WINDOW))
...
(DEFWINDOW-RESOURCE BACKGROUND-TYPEOUT-WINDOWS ()
  :MAKE-WINDOW (BACKGROUND-TYPEOUT-WINDOW
		 :HEIGHT (TRUNCATE (TV:SHEET-HEIGHT TV:DEFAULT-SCREEN) 3))
  :REUSABLE-WHEN :DEACTIVATED
  :INITIAL-COPIES 0)
...
(DEFMETHOD (ZWEI-FRAME :AFTER :CHANGE-OF-DEFAULT-FONT)
  ...
  (MULTIPLE-VALUE-SETQ (NEW-INSIDE-LEFT NEW-INSIDE-TOP NEW-INSIDE-RIGHT NEW-INSIDE-BOTTOM)
    (SEND SELF :INSIDE-EDGES-WITHOUT-MODE-LINE-WINDOW))
  (SETQ NEW-INSIDE-WIDTH (- NEW-INSIDE-RIGHT NEW-INSIDE-LEFT)
	NEW-INSIDE-HEIGHT (- NEW-INSIDE-BOTTOM NEW-INSIDE-TOP))
  (TV:WITH-SHEET-DEEXPOSED (SELF)
...
(DEFMETHOD (ZMACS-FRAME :AFTER :INIT) (IGNORE)
  ...
  (SEND TV:PROCESS :RESET)
  (SEND SELF :SELECT-PANE (SEND SELF :CREATE-WINDOW 'ZMACS-WINDOW-PANE
...
(DEFFLAVOR EDITOR-WINDOW-WITH-POP-UP-MINI-BUFFER-MIXIN
  ...
  (:DOCUMENTATION "Makes ZWEI windows work with a pop-up mini buffer."))
...
(DEFMETHOD (EDITOR-WINDOW-WITH-POP-UP-MINI-BUFFER-MIXIN :BEFORE :PREPARE-FOR-REDISPLAY)
  ...
  (LET ((*TYPEIN-WINDOW* (SEND MODE-LINE-WINDOW ':TYPEIN-WINDOW)))
    (COND ((TV:SHEET-EXPOSED-P *TYPEIN-WINDOW*)
	   ...
	   (COND ((MEMQ (SEND *TYPEIN-WINDOW* ':TYPEIN-STATUS)
			'(:CLEAR :USED))
...
(DEFFLAVOR ZWEI-WINDOW-WITH-TOP-OUTSIDE-LABEL () (TV:TOP-LABEL-MIXIN ZWEI-WINDOW)
  (:DOCUMENTATION "This flavor is useful for the COMPLETING-READ function"))
...
(DEFMETHOD (BASIC-STANDALONE-EDITOR-FRAME :SET-SIZE-FROM-STRING)
  ...
  (LET ((PANE (SEND SELF :SELECTED-PANE)))
    (MULTIPLE-VALUE-SETQ (NIL DHT NIL DWT)
      (SEND PANE :COMPUTE-MOTION STRING 0 NIL 0 0 T 0 #o5000000 #o5000000 #o5000000))
