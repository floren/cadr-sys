...
MINI-BUFFER-INITIAL-CONTENTS is a string to initialize the mini buffer with,
 MINI-BUFFER-INITIAL-CHAR-POS an index in that string to start the cursor out at."
  (MAKE-DEFAULTED-PATHNAME (READ-UNDEFAULTED-PATHNAME-STRING PROMPT
							     DEFAULTS
							     SPECIAL-TYPE
							     SPECIAL-VERSION
							     DIRECTION
...
(DEFUN READ-UNDEFAULTED-PATHNAME-STRING
  ...
  (MULTIPLE-VALUE-BIND (NIL NIL INTERVAL)	;Ok, now we can get the pathname.
      (EDIT-IN-MINI-BUFFER *PATHNAME-READING-COMTAB*
			   MINI-BUFFER-INITIAL-CONTENTS
			   MINI-BUFFER-INITIAL-CHAR-POS
			   (LIST 
			     (FORMAT NIL "~A (Default is ~A)"
				     PROMPT
...
(DEFUN UNDO-ALL-MODES ()			;Undoes all the modes currently in effect.
  (DO NIL					;It's like this in case the modes have hooks.
...
has just been done.  PATHNAME specifies host, device, directory and name to list."
  (COND ((OR (AND (EQ TYPE ':READ) (MEMQ *AUTO-DIRECTORY-DISPLAY* '(:READ T)))
	 ...
	 (SEND *DIRECTORY-LISTER* (FUNCALL PATHNAME ':NEW-PATHNAME ':TYPE ':WILD
								   ':VERSION ':WILD)))))
...
(DEFSUBST 2-DIGIT-INTO-ARRAY (ARRAY NUMBER)
  (ARRAY-PUSH-EXTEND ARRAY (+ #/0 (FLOOR NUMBER 10.)))
  (ARRAY-PUSH-EXTEND ARRAY (+ #/0 (\ NUMBER 10.))))
...
;	 (SEND PATHNAME ':NEW-DIRECTORY TYPEIN))
	(T
