...
(DEFUN PL1-FIND-START-PREV-STA (BP)
  (PROG (PREV-STA INCOMPLETE-FLAG)
	...
	(PL1-SKIP-BLANKS BP)
	(RETURN BP (BP-INDEX BP) PREV-STA INCOMPLETE-FLAG)))
...
(DEFUN READ-PL1DCL ( &AUX LINE EOFP)
  (SETQ *PL1DCL* (MAKE-ARRAY 100. ':LEADER-LIST '(0)))
  (WITH-OPEN-FILE (STREAM "SYS: ZWEI; PL1DCL LISP >" '(READ))
    (DO () (NIL)
      (MULTIPLE-VALUE (LINE EOFP)
	(FUNCALL STREAM ':LINE-IN))
      (AND EOFP (RETURN NIL))
      (ARRAY-PUSH-EXTEND *PL1DCL* LINE))))
