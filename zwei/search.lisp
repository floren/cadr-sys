		...
		(COND ((OR CRLEADS
			   (AND ( START-INDEX 0)
				(IF *ALPHABETIC-CASE-AFFECTS-SEARCH*
				    (CHAR= CHAR1 (CLI:AREF LINE START-INDEX))
				    (CHAR-EQUAL CHAR1 (CLI:AREF LINE START-INDEX)))))
			 ...
			 (CHARMAP ((CREATE-BP LINE START-INDEX)
			   ...
			   (UNLESS 
			     (IF *ALPHABETIC-CASE-AFFECTS-SEARCH*
				 (CHAR= (CHARMAP-CHARACTER) (CLI:AREF STRING I))
				 (CHAR-EQUAL (CHARMAP-CHARACTER) (CLI:AREF STRING I)))
			     (CHARMAP-RETURN NIL))
			   (INCF I))))))
	      (AND CRLEADS (EQ LINE LAST-LINE)
		   (RETURN (IF FIXUP-P (COPY-BP LIMIT-BP) NIL))))))
      (SETQ NLPOS (STRING-REVERSE-SEARCH-CHAR #/NEWLINE STRING))
      (LET ((CHARL (CLI:AREF STRING (1- STRING-LENGTH)))
	...
	(BLOCK LUPO
	  (DO ((LINE (IF (> (BP-INDEX BP) START-INDEX)
	    ...
	    (COND ((OR CRTRAILS (AND (> (LINE-LENGTH LINE) START-INDEX)
				     (IF *ALPHABETIC-CASE-AFFECTS-SEARCH*
					 (CHAR= CHARL (CLI:AREF LINE START-INDEX))
				         (CHAR-EQUAL CHARL (CLI:AREF LINE START-INDEX)))))
		   (LET ((I (1- STRING-LENGTH)))
		     (RCHARMAP ((CREATE-BP LINE (1+ START-INDEX))
		       ...
		       (IF (IF *ALPHABETIC-CASE-AFFECTS-SEARCH*
			       (CHAR= (RCHARMAP-CHARACTER) (CLI:AREF STRING I))
			       (CHAR-EQUAL (RCHARMAP-CHARACTER) (CLI:AREF STRING I)))
			   (DECF I)
			 (RCHARMAP-RETURN NIL))))))))))))

...
The remaining args are as for SEARCH."
  ...
  (DO ((I 0 (1+ I)))
      ((OR ( I LEN)
	   (= (WORD-SYNTAX (CLI:AREF KEY I)) WORD-ALPHABETIC))
       (UNLESS (ZEROP I)
		 ...
		 (BLOCK MATCH-REMAINING-WORDS
		   (DO ((I IDX)			;I index of character in line
			...
			(LLEN1 LLEN))
		     ...
		     (DO-FOREVER
		       (AND (OR ( I LLEN1)
				( (WORD-SYNTAX (CLI:AREF LINE1 I)) WORD-ALPHABETIC))
			    (RETURN NIL))
		       (INCF I))
		     ...
		     (DO-FOREVER
		       (COND (( J LEN)
			     ...
			     ((= (WORD-SYNTAX (CLI:AREF KEY J)) WORD-ALPHABETIC)
			      (RETURN NIL)))
		       (INCF J))
			 ...
			 (NIL)
		       ...
		       (SETQ CH1 (IF ( (INCF I) LLEN1) #/NEWLINE (CLI:AREF LINE1 I)))
		       (SETQ CH2 (IF ( (INCF J) LEN) #/NEWLINE (CLI:AREF KEY J))))))
...
(DEFUN PARSE-EXTENDED-SEARCH-STRING-SYNTAX (STRING INDEX LENGTH &OPTIONAL NOT-TOP-LEVEL
  ...
  (IF ( INDEX LENGTH)
      ...
      (VALUES (IF (ZEROP (CHAR-FONT (SETQ CH (CLI:AREF STRING INDEX))))
