...
;;; This is `new' MTSTR file.  It was taken from the Lambda MT sources, MTSTR LISP 92.
;;; It is (will be) merged with the CADR MT source MTSTR LISP 85.
;;; It seems that a lot of this stuff will only be applicable for local streams.

;;; Revisions:
;;;  1/04/84 17:39:34 RpK	Seems ok, check for hardware dep later...
;;;  1/10/84 21:38:59 RpK	Add changes from older MTSTR (22.x patches)
...
;;; Lossages:
;;; MAKE-MT-FILE-STREAM not generalised for remote tape

;;; To do:
;;; Streams that correctly handle block spacing (somewhat like :SET-POINTER operations)
...
(defmethod (mt-mixin :after :init) (ignore)
  "Canonicalise the UNIT instance variable."
  (setq unit (parse-unit unit)))
...
(DEFMETHOD (MT-INPUT-MIXIN :NEXT-INPUT-BUFFER)
	   (&OPTIONAL IGNORE &AUX BUFFER BYTES-TRANSFERRED USTATUS)
  (COND ((EQ STATUS ':CLOSED)
	...
	(T (OR RQB
	   ...
	   (SETQ USTATUS (SEND UNIT ':I-READ-BUFFER RQB RECORD-SIZE DENSITY))
	   ;; [OLD/CADR] (MT-RUN RQB %MT-COMMAND-READ (MINUS RECORD-SIZE) UNIT DENSITY)
	   (COND ((SEND UNIT ':STATUS-EOF USTATUS)
		  (SETQ STATUS ':EOF)
		  NIL)
		 (T (SETQ BYTES-TRANSFERRED
			  (SEND UNIT ':DECODE-STATUS USTATUS ':BYTES-TRANSFERRED))
; [OLD/CADR]		  (- (SIGN-EXTEND-16 (AR-1 RQB %MT-BYTE-COUNT-AFTER))
;			     (SIGN-EXTEND-16 (AR-1 RQB %MT-BYTE-COUNT))))
		    (SETQ BUFFER (MT-GET-RQB-ARRAY RQB BYTE-SIZE))
		    (UNLESS (SEND UNIT ':ODD-BYTES-OK-P)
		      ;; designed to catch CADR lossage
		    ...
		    (VALUES BUFFER 0 (// (* 8 BYTES-TRANSFERRED) BYTE-SIZE)))))))
...
(DEFMETHOD (MT-OUTPUT-MIXIN :SEND-OUTPUT-BUFFER) (BUFFER TO-INDEX)
  (OR (AND (ARRAY-INDIRECT-P BUFFER)
      ...
      (FERROR NIL "Attempt to :SEND-OUTPUT-BUFFER ~S, which is not indirected into ~S"
	      BUFFER RQB))
  (LET ((COUNT (// (* TO-INDEX BYTE-SIZE) 8)))
    ...
    (COND ((BIT-TEST COUNT 3)
	   ...
	   (INCF COUNT 2)))
    (COND ((PLUSP COUNT)
	   (SEND UNIT ':I-WRITE-BUFFER RQB COUNT DENSITY IBM-MODE)))))
;; [OLD]   (MT-RUN RQB %MT-COMMAND-WRITE (MINUS COUNT) UNIT DENSITY IBM-MODE)))))
...
(DEFUN MAKE-MT-STREAM (&OPTIONAL &KEY (DIRECTION ':INPUT) (BYTE-SIZE 8) (CHARACTERS T)
  ...
  (SETQ UNIT (PARSE-UNIT UNIT))
  (MAKE-INSTANCE (SEND UNIT :STREAM-FLAVOR DIRECTION CHARACTERS ())
		 ':BYTE-SIZE BYTE-SIZE
		 ':UNIT UNIT
...
(DEFUN MAKE-MT-FILE-STREAM (&OPTIONAL
  ...
  (COND ((EQ DIRECTION ':INPUT)
	 ...
	 (IF (NULL PLIST)
	     ...
	     (MAKE-INSTANCE (SEND UNIT :STREAM-FLAVOR :INPUT CHARACTERS T)
			    ':BYTE-SIZE BYTE-SIZE
			    ':UNIT UNIT
			    ':RECORD-SIZE RECORD-SIZE
			    ':DENSITY DENSITY
	...
	((EQ DIRECTION ':OUTPUT)
	 ...
	 (WRITE-MAGTAPE-HEADER
	   ...
	   HEADER-STYLE)
	 (FUNCALL STREAM ':CLOSE ':RAW)
	 (MAKE-INSTANCE (SEND UNIT :STREAM-FLAVOR :OUTPUT CHARACTERS T)
			':BYTE-SIZE BYTE-SIZE
			':UNIT UNIT
			':RECORD-SIZE RECORD-SIZE
			':DENSITY DENSITY
...
(DEFUN MT-OPEN-ERROR (CONDITION ERROR-P)
  (IF ERROR-P (SIGNAL-CONDITION CONDITION) CONDITION))
