...
(DEFCONST %QFILE-CHARACTER-OPCODE CHAOS:DAT-OP)
(DEFCONST %QFILE-BINARY-OPCODE (LOGIOR CHAOS:DAT-OP 100))
(DEFCONST %QFILE-COMMAND-OPCODE CHAOS:DAT-OP)
(DEFCONST %QFILE-SYNCHRONOUS-MARK-OPCODE (1+ CHAOS:DAT-OP))
(DEFCONST %QFILE-ASYNCHRONOUS-MARK-OPCODE (+ CHAOS:DAT-OP 2))
(DEFCONST %QFILE-NOTIFICATION-OPCODE (+ CHAOS:DAT-OP 3))
(DEFCONST %QFILE-EOF-OPCODE CHAOS:EOF-OP)
...
(DEFINE-FILE-ACCESS QFILE-ACCESS .9S0 (:NETWORK :CHAOS))
...
WHOSTATE is what to say in the who-line while waiting."
  (IF (NULL (SETQ ID (ASSOC TID *QFILE-PENDING-TRANSACTIONS*)))
  ...
  (CASE REASON
    (:INPUT
	 ...
	 (%QFILE-COMMAND-OPCODE
	  ...
	  (SETQ TEM (ASSOC STRING *QFILE-PENDING-TRANSACTIONS*))
...
(DEFUN QFILE-PROCESS-ERROR-NEW (STRING &OPTIONAL PATHNAME-OR-STREAM
			       ...
			       (DEFAULT-CONS-AREA SYS:BACKGROUND-CONS-AREA))
  (COND ((TYPEP PATHNAME-OR-STREAM 'fs:PATHNAME)
  ...
  (LEXPR-FUNCALL #'FILE-PROCESS-ERROR (GET (INTERN ERROR-CODE 'FS) 'FS:FILE-ERROR)
...
Gets an error if the reply's command name is not COMMAND."
  ...
  (SETQ END (OR (STRING-SEARCH-SET '(#/SP #/NEWLINE) RETURNED-STRING START)
	  ...
	  ((SETQ PKT (FILE-WAIT-FOR-TRANSACTION TRANSACTION-ID CONTROL-CONNECTION WHOSTATE))
	   ...
	   (SETQ SUCCESS (LET ((FROM (IF HANDLE
					 (QFILE-CHECK-HANDLE HANDLE STRING)
				         (1+ (STRING-SEARCH-SET '(#/SP #/NEWLINE)
								STRING)))))
...
If it does, returns the index of the first character in STRING after the handle."
  (LET ((HANDLE-END (STRING-SEARCH-SET '(#/SP #/NEWLINE) STRING)))
    (AND (NULL HANDLE-END)
    ...
    (1+ HANDLE-END)))

(DEFUN QFILE-LOGIN-HOST-UNIT
       (UNIT LOGIN-P UNAME-HOST &AUX HOST CONN SUCCESS PKT ENABLE-CAPABILITIES
	(DEFAULT-CONS-AREA SYS:BACKGROUND-CONS-AREA))
  ...
  (AND CONN (EQ (CHAOS:STATE CONN) 'CHAOS:OPEN-STATE)
	 ...
	 (DO ((ID (FILE-MAKE-TRANSACTION-ID))
	   ...
	   (UNLESS (EQ (CHAOS:STATE CONN) 'CHAOS:OPEN-STATE)
		...
		(FERROR 'fs:HOST-NOT-AVAILABLE "File server ~2@*~A is refusing file connections."
			NIL NIL HOST))))
...
(DEFUN READ-FILE-PROPERTY-LIST-STRING (STRING OPERATION PATHNAME
  ...
  (DO ((I (QFILE-CHECK-COMMAND OPERATION STRING))
       ...
       (*READ-BASE* 10.) (*READTABLE* SI:INITIAL-COMMON-LISP-READTABLE)
       (TYPE) (DATE-START))
...
(DEFUN RENAME-CHAOS (ACCESS OLD-PATHNAME NEW-PATHNAME ERROR-P
		     &AUX HOST-UNIT PKT SUCCESS STRING)
  (FILE-OPERATION-RETRY
    (SETQ HOST-UNIT (SEND ACCESS :GET-HOST-UNIT))
    (MULTIPLE-VALUE (PKT SUCCESS STRING)
	      ...
	      (COND (FROM
		     (SETQ TRUENAME-STRING
			   (SUBSTRING STRING (1+ FROM)
				      (STRING-SEARCH #/NEWLINE STRING (1+ FROM))))
		     (FS:PARSE-PATHNAME TRUENAME-STRING (SEND OLD-PATHNAME :HOST)))
		    (T T)))
	  (QFILE-PROCESS-ERROR-NEW STRING OLD-PATHNAME NIL (NOT ERROR-P) :RENAME))
      (CHAOS:RETURN-PKT PKT))))

(DEFUN COMPLETE-CHAOS (ACCESS PATHNAME STRING OPTIONS
		       &AUX HOST-UNIT STRING-ORIGIN
			    DELETED-P WRITE-P NEW-OK
			    (DEFAULT-CONS-AREA SYS:BACKGROUND-CONS-AREA))
  (DOLIST (KEY OPTIONS)
    (CASE KEY
      (:DELETED
  ...
  (SETQ HOST-UNIT (SEND ACCESS :GET-HOST-UNIT))
  (MULTIPLE-VALUE-BIND (PKT SUCCESS FILE-STRING)
      (SEND HOST-UNIT :COMMAND NIL NIL NIL "Complete"
	    	      "COMPLETE"
    ...
    (WHEN SUCCESS
      ...
      (SETQ SUCCESS (LET ((*PACKAGE* SI:PKG-KEYWORD-PACKAGE))
		      (READ-FROM-STRING FILE-STRING NIL
					(QFILE-CHECK-COMMAND "COMPLETE" FILE-STRING))))
...
(DEFUN CHANGE-PROPERTIES-CHAOS (ACCESS PATHNAME ERROR-P PROPERTIES
  ...
  (FILE-OPERATION-RETRY
    ...
    (MULTIPLE-VALUE (PKT SUCCESS STRING)
      (SEND HOST-UNIT :COMMAND NIL NIL NIL "Change Properties"
	    	      STRING))
    (UNWIND-PROTECT
      (UNLESS SUCCESS
...
(DEFUN DIRECTORY-OPERATION-CHAOS (OPERATION ACCESS PATHNAME ERROR-P
				  &AUX HOST-UNIT PKT SUCCESS FILE-STRING)
  (FILE-OPERATION-RETRY
    (SETQ HOST-UNIT (SEND ACCESS :GET-HOST-UNIT))
    (MULTIPLE-VALUE (PKT SUCCESS FILE-STRING)
      (SEND HOST-UNIT :COMMAND NIL NIL NIL "Directory Frob"
    ...
    (UNWIND-PROTECT
      (COND (SUCCESS
	    ...
	    (T
	     (QFILE-PROCESS-ERROR-NEW FILE-STRING PATHNAME NIL (NOT ERROR-P) OPERATION)))
      (CHAOS:RETURN-PKT PKT))))

(DEFUN CHANGE-CAPABILITIES-CHAOS (ACCESS CAPABILITIES ENABLE-P
				  &OPTIONAL (HOST-UNIT NIL HOST-UNIT-SPECD)
				  &AUX PKT SUCCESS FILE-STRING COMMAND WHOSTRING
				  CAP-STRING CURRENT-CAPABILITIES (HOST (SEND ACCESS :HOST)))
  (OR HOST-UNIT-SPECD (SETQ HOST-UNIT (SEND ACCESS :GET-HOST-UNIT)))
  (SETQ CURRENT-CAPABILITIES (GET HOST-CAPABILITIES-PLIST HOST))
  (IF ENABLE-P
      (SETQ COMMAND "ENABLE-CAPABILITIES"
	    WHOSTRING "Enable Capabilities")
      (SETQ COMMAND "DISABLE-CAPABILITIES"
	    WHOSTRING "Disable Capabilities"))
  ...
  (MULTIPLE-VALUE (PKT SUCCESS FILE-STRING)
    (SEND HOST-UNIT :COMMAND NIL NIL NIL WHOSTRING COMMAND #/SP CAP-STRING))
  (UNWIND-PROTECT
    (WHEN SUCCESS
	...
	(SETQ CURRENT-CAPABILITIES
      ...
      (PUTPROP HOST-CAPABILITIES-PLIST CURRENT-CAPABILITIES HOST)
	  ...
	  (LET ((UNITS (SEND ACCESS :HOST-UNITS)))
	    (DOLIST (UNIT UNITS)
	      (AND (NEQ UNIT HOST-UNIT)
		   ...
		   (SEND UNIT :COMMAND NIL NIL NIL WHOSTRING
			      COMMAND #/SP CAP-STRING))))))
    (CHAOS:RETURN-PKT PKT)))

(DEFUN CWD-CHAOS (ACCESS PATHNAME ERROR-P ACCESS-P &OPTIONAL (HOST-UNIT NIL HOST-UNIT-SPECD)
  ...
  (DO-FOREVER
	...
	(COND (SUCCESS
	       ...
	       (PUTPROP (IF ACCESS-P HOST-ACCESSED-DIRECTORY-PLIST
			  HOST-CONNECTED-DIRECTORY-PLIST)
			PATHNAME HOST)
	       ;; Also inform any other host units that are connected now.
	       (OR HOST-UNIT-SPECD
		   (LET ((UNITS (SEND ACCESS :HOST-UNITS)))
    ...
    (CONDITION-CASE-IF ACCESS-ERROR (ERROR-OBJECT)
	 ...
	 (PROGN
	   (MULTIPLE-VALUE (PKT SUCCESS STRING)
	     ...
	     (IF (TYPEP FILE 'FS:(OR LMFILE-PARSING-MIXIN LM-PARSING-MIXIN))
  ...
  (DO ((LIST-TO-DO FILE-LIST (CDR LIST-TO-DO))
	...
	(AND TRANSACTION-ID
	     (LET* ((PKT (FILE-WAIT-FOR-TRANSACTION TRANSACTION-ID CONNECTION "Probe"))
		    ...
		    (FROM (1+ (STRING-SEARCH-SET '(#/SP #/NEWLINE) STRING)))
...
(DEFUN MAKE-FILE-PROPERTY-LIST-STREAM-CHAOS (ACCESS COMMAND WHOSTATE
					     STRING-ARG TOKEN-ARGS PATHNAME NOERROR-P
  ...
  (UNWIND-PROTECT
	...
	(COND ((NOT SUCCESS)
	       ...
	       (QFILE-PROCESS-ERROR-NEW STRING PATHNAME NIL NOERROR-P :DIRECTORY-STREAM))
	      (T
	       (QFILE-CHECK-COMMAND COMMAND STRING)
	       (CHAOS:RETURN-PKT PKT)
	       (PROG1
		 (MAKE-INSTANCE 'QFILE-DIRECTORY-STREAM
				:HOST-UNIT HOST-UNIT
				:DATA-CONNECTION DATA-CONN
				:PATHNAME PATHNAME)
		 (SETQ NOT-ABORTED T)))))
	...
	(CONDITION-CASE ()
		...
		(SEND HOST-UNIT :COMMAND NIL (DATA-INPUT-HANDLE DATA-CONN) NIL "Close"
		      		"CLOSE")
	      (WHEN CLOSE-SUCCESS
...
(DEFMETHOD (QFILE-STREAM-MIXIN :QFASLP) () (GET (LOCF SI:PROPERTY-LIST) :QFASLP))

(DEFMETHOD (QFILE-STREAM-MIXIN :TRUENAME) () (GET (LOCF SI:PROPERTY-LIST) :TRUENAME))

(DEFMETHOD (QFILE-STREAM-MIXIN :LENGTH) () (GET (LOCF SI:PROPERTY-LIST) :LENGTH))
...
(DEFMETHOD (QFILE-INPUT-STREAM-MIXIN :REAL-CLOSE) (ABORTP &AUX SUCCESS STRING)
  ...
  (COND (SUCCESS
	 ...
	 (IF (AND STRING
		  (STRING-SEARCH-CHAR #/NEWLINE STRING))
       ...
       (COND (SUCCESS
		...
		(COND (FROM
		       (SETQ TRUENAME-STRING
			     (SUBSTRING STRING (1+ FROM)
					(STRING-SEARCH #/NEWLINE STRING (1+ FROM))))
		       (SEND SELF :PUTPROP (FS:PARSE-PATHNAME TRUENAME-STRING
...
(DEFFLAVOR QFILE-BINARY-STREAM-MIXIN () (QFILE-DATA-STREAM-MIXIN))
...
(DEFFLAVOR QFILE-INPUT-SIGNED-BINARY-STREAM-MIXIN (CURRENT-BYTE-SIZE)
	   (QFILE-INPUT-STREAM-MIXIN QFILE-BINARY-STREAM-MIXIN)
  (:REQUIRED-FLAVORS SI:BASIC-BUFFERED-INPUT-STREAM)
  :INITTABLE-INSTANCE-VARIABLES)

(DEFMETHOD (QFILE-INPUT-SIGNED-BINARY-STREAM-MIXIN :AFTER :INIT) (IGNORE)
  (SETQ CURRENT-BYTE-SIZE (GET (LOCF SI:PROPERTY-LIST) :BYTE-SIZE)))

(DEFMETHOD (QFILE-INPUT-SIGNED-BINARY-STREAM-MIXIN :AFTER :SET-BYTE-SIZE) (NEW-BYTE-SIZE)
...
(DEFMETHOD (QFILE-ACCESS :ENABLE-CAPABILITIES) (CAPABILITIES &OPTIONAL UNIT)
  (IF UNIT (CHANGE-CAPABILITIES-CHAOS SELF CAPABILITIES T UNIT)
    (CHANGE-CAPABILITIES-CHAOS SELF CAPABILITIES T)))

(DEFMETHOD (QFILE-ACCESS :DISABLE-CAPABILITIES) (CAPABILITIES &OPTIONAL UNIT)
  (IF UNIT (CHANGE-CAPABILITIES-CHAOS SELF CAPABILITIES NIL UNIT)
